...
"<small id=msg>支持 jpg / gif，可多选。jpg 自动裁剪为 360x360，gif 保留动图直接上传</small>"
"</div>"
...
"function startCrop(){let fs=document.getElementById('f').files;if(!fs.length){alert('请先选择图片');return}cropFiles=Array.from(fs);cropIdx=0;cropQueue=[];processNextFile()}"
"function processNextFile(){if(cropIdx>=cropFiles.length){uploadCropped();return}let file=cropFiles[cropIdx];if(file.type==='image/gif'){cropQueue.push({blob:file,name:'crop_'+Date.now()+'.gif'});cropIdx++;processNextFile();}else{showCropImage(file)}}"
